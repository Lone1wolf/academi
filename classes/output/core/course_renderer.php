    ...
    public function frontpage_available_courses() {
        ...
        if (!empty($courses)) {
            ...
            foreach ($courses as $course) {
                ...
                $categoryID = $course->category; // Assuming $course->category contains the category ID
                $category = \core_course_category::get($categoryID);
                $courseCategory = $category->get_formatted_name();

                $courseName = $course->fullname;
                $courseShortName = $course->shortname;
                $courseStartDate = date('Y-m-d', $course->startdate); // Assuming 'startdate' is in UNIX timestamp format
                $courseEndDate = date('Y-m-d', $course->enddate); // New
                $courseLanguage = $course->lang ? $course->lang : 'pt';
                $courseSummary = $course->summary;
                $summaryText = strip_tags($courseSummary);
                $limitedSummary = substr($summaryText, 0, 50); 
                $noimgurl = $this->output->image_url('no-image', 'theme');

                // $courseImageURL = \core_course\external\course_summary_exporter::get_course_image($courseID);
                ...
                // Attempt to fetch the course image URL using another method
                // Replace this logic with appropriate code to get the image URL
                // For instance, you might use $course->id to retrieve the URL from the course files, database, or any custom logic.
                // $courseImageURL = get_course_image_url_using_custom_logic($course->id);
        
                // Create an array for each course including additional attributes
                $courseData = [
                    'name' => $courseName,
                    'id' => $courseID,
                    'link' => $courseLink,
        ...
        // $data['imgurl'] = $noimgurl;
                ...
                // Push the course data into the $data array
                $data[] = $courseData;
            }
